...
def generate(output_file="SpatialStandardGenerator_output_file.txt", time_frames_number=10, area=1000, cell_size=5, n_colloc=3, lambda_1=5, lambda_2=100, m_clumpy=1, m_overlap=1, ncfr=1.0, ncfn=1.0, ndf=2, ndfn=5000, random_seed=None):
    print("generate()")
    ...
    for i_time_frame in range(time_frames_number):
        ...
        for i_colloc in range(n_colloc * m_overlap):
            ...
            time_frame_ids = np.ones(shape=collocation_features_instances_sum, dtype=np.int32) * i_time_frame
        ...
        time_frame_ids = np.ones(shape=collocation_noise_features_instances_sum, dtype=np.int32) * i_time_frame
        ...
        if ndf > 0:
            ...
            time_frame_ids = np.ones(shape=ndfn, dtype=np.int32) * i_time_frame
